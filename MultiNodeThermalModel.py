...
# Rod properties
CARod = 0.0032*np.pi*0.0023 # m², contact area of the rod with the frame
...
NodeEarth = Node(emissivity=emSol, absorptivity=alSol, Neigbors=[(6, CASFrame, 400)], ID=0, m=0.035, cp=896, Area=Area, F=F, Qgen=0, Adjust=1)
NodeEarth.Toggle = [0, 1, 1, 0, 1]

NodeSun  = Node(emissivity=emSol, absorptivity=alSol, Neigbors=[(6, CATFrame, 400)], ID=1, m=0.035, cp=896, Area=Area, F=0, Qgen=0, Adjust=1)
NodeSun.Toggle = [1, 0, 0, 0, 1]

Nodev = Node(emissivity=emSol, absorptivity=alSol, Neigbors=[(6, CASFrame, 400)], ID=2, m=0.035, cp=896, Area=Area, F=FP, Qgen=0, Adjust=0)
Nodev.Toggle = [0, 1, 1, 0, 1]

Nodenv = Node(emissivity=emSol, absorptivity=alSol, Neigbors=[(6, CASFrame, 400)], ID=3, m=0.035, cp=896, Area=Area, F=FP, Qgen=0, Adjust=0)
Nodenv.Toggle = [1, 1, 1, 0, 1]

NodeS = Node(emissivity=emSol, absorptivity=alSol, Neigbors=[(6, CASFrame, 400)], ID=4, m=0.038, cp=896, Area=Area, F=FP, Qgen=0,BetaAdjust=0)
NodeS.Toggle = [0, 1, 1, 0, 1]

NodeN = Node(emissivity=emSol, absorptivity=alSol, Neigbors=[(15, 0.1*0.1, 400)], ID=5, m=0.038, cp=896, Area=Area, F=FP, Qgen=0)
...
NodeRod1 = Node(emissivity=0, absorptivity=0, Neigbors=[
    (6, CARod*2, 400), (11, CASub, 400), (12, CASub, 400), (13, CASub, 400), (14, CASub, 400)
...
NodeOBC = Node(emissivity=0, absorptivity=0, Neigbors=[
    (7, CASub, 400), (8, CASub, 400), (9, CASub, 400), (10, CASub, 400), (15, 2*CABS, 400), (13,CABS, 400)
], ID=14, m=0.159, cp=896, Area=0, F=FP, Qgen=0.17*1.25)
NodeOBC.Toggle = [0, 0, 0, 1, 0]

NodePayloadII = Node(emissivity=0, absorptivity=0, Neigbors=[
    (7, CASub, 400), (8, CASub, 400), (9, CASub, 400), (10, CASub, 400), (14, CABS, 400), (12, CABS, 400)
], ID=13, m=0.024, cp=896, Area=0, F=FP, Qgen=0)#0.125)
NodePayloadII.Toggle = [0, 0, 0, 1, 0]

NodeEPS = Node(emissivity=0, absorptivity=0, Neigbors=[
    (7, CASub, 400), (8, CASub, 400), (9, CASub, 400), (10, CASub, 400), (11, CABS, 400), (13, CABS, 400)
], ID=12, m=0.086, cp=896, Area=0, F=FP, Qgen=0)
NodeEPS.Toggle = [0, 0, 0, 0, 0]

NodePayloadI = Node(emissivity=0, absorptivity=0, Neigbors=[
    (7, CASub, 400), (8, CASub, 400), (9, CASub, 400), (10, CASub, 400), (12, CABS, 400)
], ID=11, m=0.031, cp=896, Area=0, F=FP, Qgen=0)#0.00026)
NodePayloadI.Toggle = [0, 0, 0, 1, 0]

NodeFrame = Node(emissivity=0, absorptivity=0.0, Neigbors=[
    (1, CASFrame, 400), (2, CASFrame, 400), (3, CASFrame, 400), (4, CASFrame, 400), (0, CASFrame, 400),
    (7, CARod, 400), (8, CARod, 400), (9, CARod, 400), (10, CARod, 400), (15, CATFrame, 400)
], ID=6, m=0.130, cp=896, Area=AreaFrame, F=FP, Qgen=0)
...
for t in timerange:
    ...
    for idx, node in enumerate(Nodes):
        node.TemperatureChange(timestep, cp=node.cp, m=node.m, S=(1367/(r(theta(t))/astunit)**2), beta=beta, albedo=albedo(beta), t=t)  # Update temperature
...
plt.xlabel('Orbits [n]',fontsize= 20)
...
#plt.title('Selected Node Temperatures Over Time')
plt.tick_params(axis='both', labelsize=14)  # 14 is the font size
plt.xlim(14*OrbitalPeriod,16*OrbitalPeriod) 
plt.legend()
plt.grid(True)
...
for idx, node in enumerate(Nodes):
    for neighbor in node.Neighbors:
        neighbor_id = neighbor[0] # gay boy
...
plt.title("Thermal Node Connection Graph (Unique Node & Edge Colors)")
plt.axis('off')
plt.show() 

import matplotlib.pyplot as plt

# Assume you have: times, and for each node, a list of toggle states over time (e.g., solar_toggles[node_idx][t_idx])
# We'll track the solar toggle (Toggle[0]) for each node at each timestep
